...
1. Determine whether a corresponding test file exists in the matching `*_tests/` tree per `.cursor/rules/public-api-tests.mdc`.
2. Determine whether it is in the **correct tier** for that module's domain (e.g. core decorators → `dagster_tests/definitions_tests/decorators_tests/`; libraries → `python_modules/libraries/<pkg>/<pkg_underscore>_tests/`; annotation mechanics → `dagster_tests/utils_tests/test_annotations.py`).
3. Record `test_tier_status` per module as one of: `present_correct_tier`, `present_wrong_tier`, `missing`.
4. If tests exist and can be scoped, run:
...
## MISSING
…   # missing root exports and/or missing expected test paths / wrong-tier tests
